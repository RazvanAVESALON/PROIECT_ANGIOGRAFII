...
from custom_net import CustomNet  , Resnet18
    ...
    for ep in range(epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            with tqdm(desc=phase, unit=' batch', total=len(loaders[phase].dataset)) as pbar:
                ...
                exp.log_metrics({f"{phase}MSE": mse_value, f"{phase}loss": loss_value}, epoch=ep)
...
def main():
    ...
    geometric_t = TR.Compose([
        ...
        TR.RandFlipd(keys=["img", "seg"],prob=config['train']['flip_prob'], spatial_axis=config['train']['flip_spatial_axis'] ),
  ...
  # Specificarea functiei loss
    ...
    history = train(network, train_loader, valid_loader, experiment ,criterion, opt, epochs=config['train']['epochs'], thresh=config['test']['threshold'], weights_dir=path)
